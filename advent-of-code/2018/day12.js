...
function populatePots() {
  return Array(INITIAL_POTS_STATE.length + OFFSET_LEFT + OFFSET_RIGHT)
    ...
    .map((x, i) => {
      if (i >= OFFSET_LEFT) {
        return INITIAL_POTS_STATE[i - OFFSET_LEFT] || x;
      }
      return x;
    });
}
...
of plants, a given pot has or does not have a plant based on whether that pot (and the two 
pots on either side of it) had a plant in the last generation. These are written as 
LLCRR => N, where L are pots to the left, C is the current pot being considered, R are the 
pots to the right, and N is whether the current pot will have a plant in the next generation. 
For example:
...
looking for ..#.., the one in pot 4 matched the rule looking for .#.#., 
